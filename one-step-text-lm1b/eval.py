...
Usage
-----
  scripts/run.sh python eval.py                                     # argmax, z_scale=0.90
  scripts/run.sh python eval.py --temperature 0.9 --top_p 0.95
  scripts/run.sh python eval.py --sweep                             # sweep temperatures
  scripts/run.sh python eval.py --z_scale_sweep                     # sweep z_scales
  scripts/run.sh python eval.py --reference_ppl                     # evaluator calibration check
  scripts/run.sh python eval.py --reference_ppl --num_ref 2048      # use more sentences
"""
import argparse
import math
...
from prepare import QWEN_VOCAB_SIZE, MAX_SEQ_LEN, QWEN_HIDDEN, PAD_TOKEN_ID, QWEN_MODEL
from runtime_paths import dataset_dir, resolve_checkpoint_path
from train_stage_a import StageAModel
from train_stage_b import TextGeneratorTransformer, generate_samples, decode_tokens
...
def generate_nf_decoder(stage_a: StageAModel, n: int, device: str,
    ...
    """Retained for CLI compatibility; Stage A no longer contains a decoder."""
    raise RuntimeError(
        "NF+decoder evaluation is no longer supported: Stage A is now flow-only with no decoder head."
    )
...
def load_generator(checkpoint_path: str, device: str):
    ...
    latent_dim = ckpt["generator_state_dict"]["z_proj.weight"].shape[1]
    generator = TextGeneratorTransformer(
        seq_len=MAX_SEQ_LEN, latent_dim=latent_dim, vocab_size=QWEN_VOCAB_SIZE,
        width=gc["width"], depth=gc["depth"],
        num_heads=gc["num_heads"], ffn_dim=gc["ffn_dim"],
    ).to(device)

    if "ema_state_dict" in ckpt:
        generator.load_state_dict(ckpt["ema_state_dict"])
...
def compute_reference_ppl(device: str,
    ...
    if not cache_path.exists():
        raise FileNotFoundError(
            ...
            "Run: scripts/run.sh python prepare.py --dataset lm1b --tokenizer qwen"
        )
...
def evaluate(generator, device: str,
             ...
             gpt2_model: str = "gpt2-large", batch_size: int = 16) -> dict:
    print(f"\n[eval] Generating {num_samples} samples "
          f"(temperature={temperature}, top_p={top_p}, z_scale={z_scale}) ...")
    ...
    for start in range(0, num_samples, chunk):
        ...
        ids = generate_samples(generator, n, device,
                               latent_dim=getattr(generator, "latent_dim", QWEN_HIDDEN),
    ...
    return {"temperature": temperature, "top_p": top_p, "z_scale": z_scale,
            "num_samples": num_samples, "entropy": entropy, "gen_ppl": gen_ppl}


def print_table(rows: list[dict]):
    print("\n" + "=" * 70)
    print(f"{'temperature':>12}  {'top_p':>6}  {'z_scale':>7}  {'Gen. PPL ↓':>10}  {'Entropy':>8}")
    print("-" * 70)
    target_entropy = 4.31
    for r in rows:
...
def parse_args():
    ...
    p.add_argument("--checkpoint", default="stage_b/v3_lm1b/coupling_lm1b/checkpoint.ckpt",
    ...
    p.add_argument("--z_scale", type=float, default=1.00,
                   help="Scale for z~N(0,I). TarFlow Stage A produces z_std≈1.0.")
    p.add_argument("--sweep", action="store_true",
                   help="Sweep temperatures [0, 0.7, 0.8, 0.9, 1.0] with top_p=0.95")
    p.add_argument("--z_scale_sweep", action="store_true",
                   help="Sweep z_scales [0.60, 0.75, 0.838, 0.90, 1.00, 1.10] at argmax")
    p.add_argument("--nf_decoder", action="store_true",
                   help="Deprecated: Stage A no longer has a decoder. This flag now errors.")
    p.add_argument("--stage_a_checkpoint",
                   default="stage_a/v3_lm1b/coupling_lm1b/checkpoint.ckpt",
...
if __name__ == "__main__":
    ...
    generator, _ = load_generator(str(resolve_checkpoint_path(args.checkpoint)), device)
